template<int dim>
void
Base_Solver<dim>::run_square_circular_cavity_channel()
{
	const unsigned int refine_cycles = constants.refine_cycles;
	error_per_itr.resize(refine_cycles);

	Assert(constants.mesh_type == Mesh_type::square_circular_cavity_channel, ExcMessage("Inappropriate mesh type"));
	...
	std::cout << "Integrated inflow_outflow times " << integrate_inflow << std::endl;
