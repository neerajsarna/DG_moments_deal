namespace Test_BoundaryHandler_Odd
{
	...
	// 	// we construct a vector of all the system data being considered 
		...
		Vector<double> bc_rhs(System[0].nBC);
		Vector<double> bc_rhs_manuel(System[0].nBC);
		...
		Vector<double> wall_velocity = System[0].bcrhs_wall.local_wall_velocity(vx0,vy0,projector);
		...
		System[0].base_bcrhs->BCrhs(p,normal_vector,bc_rhs,0);

		for (unsigned int i = 0 ; i < bc_rhs.size() ; i++)
			EXPECT_NEAR(bc_rhs(i),bc_rhs_manuel(i),1e-5) << "value of i " << i ;

		wall_velocity = System[0].bcrhs_wall.local_wall_velocity(vx1,vy1,projector);
		...
		System[0].bcrhs_wall.BCrhs(p,normal_vector,bc_rhs,1);

		for (unsigned int i = 0 ; i < bc_rhs.size() ; i++)
			EXPECT_NEAR(bc_rhs(i),bc_rhs_manuel(i),1e-5) << "value of i " << i ;

			//RIGHT WALL
		wall_velocity = System[0].bcrhs_wall.local_wall_velocity(vx2,vy2,projector);
		...
		System[0].bcrhs_wall.BCrhs(p,normal_vector,bc_rhs,2);

		for (unsigned int i = 0 ; i < bc_rhs.size() ; i++)
			EXPECT_NEAR(bc_rhs(i),bc_rhs_manuel(i),1e-5) << "value of i " << i ;


		wall_velocity = System[0].bcrhs_wall.local_wall_velocity(vx3,vy3,projector);
			// TOP WALL
		...
		System[0].bcrhs_wall.BCrhs(p,normal_vector,bc_rhs,3);
