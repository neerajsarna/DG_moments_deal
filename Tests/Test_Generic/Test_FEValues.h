namespace Test_FEValues
{
 ...
 //        		std::cout << "quad points " << q << " Dof " << i << " shape_value " << fe_values.shape_value(i,q)<< std::endl;		
	...
	// the following function handles integrate difference
	TEST(IntegrateDifference,HandlesIntegrateDifference)
	{
		...
		const unsigned int components_to_mask = 1;

		FESystem<dim> finite_element(FE_DGQ<dim>(p),max_components);
		DoFHandler<dim> dof_handler(triangulation);
		...
		// zero weightage to the first component
		selected = std::make_pair(max_components-1,max_components);

		QGauss<dim> quadrature(p+1);
		ComponentSelectFunction<dim> weight(selected,max_components);          // used to compute only the error in theta

		VectorTools::integrate_difference (mapping,
    							           dof_handler,
        		                           solution,
        								  ZeroFunction<dim>(max_components),
        				 			  	  error_per_cell,
        							  	  quadrature,
        							      VectorTools::L2_norm,
        							  	  &weight);  

		std::cout << "norm of function " << error_per_cell.l2_norm() << std::endl;
