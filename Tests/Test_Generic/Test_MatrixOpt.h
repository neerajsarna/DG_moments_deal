...
namespace TestMatrixOpt
{
   ...
   0.9258200997725514,0,0.2716533280373359,-0.4129081960150717,0,0.2519763153394847;
	...
	TEST(SquareOuterProductAB,HandlesSquareOuterProductAB)
	{
		Full_matrix A(2,2);
		Full_matrix B(2,2);
		Full_matrix result(4,4);
		Full_matrix result_manuel(4,4);
		...
		result_manuel.setZero();

		result_manuel.block(0,2,2,2) = A(0,1) * B;
		result_manuel.block(2,0,2,2) = A(1,0) * B;
		result_manuel.block(2,2,2,2) = A(1,1) * B;
	...
	TEST(RectangeOuterProductAB,HandlesRectangleOuterProductAB)
	{
		Full_matrix A(2,2);
		Full_matrix B(2,3);
		Full_matrix result(4,6);
		Full_matrix result_manuel(4,6);
		...
		// manual result
		result_manuel.block(0,0,2,3) = A(0,0) * B;
		result_manuel.block(0,3,2,3) = A(0,1) * B;
		result_manuel.block(2,0,2,3) = A(1,0) * B;
		result_manuel.block(2,3,2,3) = A(1,1) * B;

		for (unsigned int i = 0 ; i < result_manuel.rows() ; i++)
			for (unsigned int j = 0 ; j < result_manuel.cols() ; j++)
				EXPECT_NEAR(result(i,j),result_manuel(i,j),1e-5);

	}

	TEST(RectangeOuterProductASparseB,HandlesRectangleOuterProductASparseB)
	{
		Full_matrix A(2,2);
		Sparse_matrix B(2,3);
		Sparse_matrix result(4,6);
		Full_matrix result_manuel(4,6);
		...
		for (unsigned int i = 0 ; i < result_manuel.rows() ; i++)
			for (unsigned int j = 0 ; j < result_manuel.cols() ; j++)
				EXPECT_NEAR(result.coeffRef(i,j),result_manuel(i,j),1e-5);
	}
	...
	TEST(RectangeOuterProductSparseAB,HandlesRectangleOuterProductSparseAB)
	{
		...
		// manual result
		result_manuel.block(0,0,2,3) = A.coeffRef(0,0) * B;
		result_manuel.block(0,3,2,3) = A.coeffRef(0,1) * B;
		result_manuel.block(2,0,2,3) = A.coeffRef(1,0) * B;
		result_manuel.block(2,3,2,3) = A.coeffRef(1,1) * B;
	...
	TEST(RectangeOuterProductSparseASparseB,HandlesRectangleOuterProductSparseASparseB)
	{
		Sparse_matrix A(2,2);
		Sparse_matrix B(2,3);
		Sparse_matrix result(4,6);
