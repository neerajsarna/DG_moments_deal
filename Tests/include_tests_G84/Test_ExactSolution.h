namespace Test_ExactSolution
{
	...
	TEST(ExactSolutionPoissonHeatG84,HandlesExactSolutionG84)
	{
		...
		if (constants.constants.mesh_type == periodic_square)
		{
			...
			ASSERT_NEAR(G84.constants.alpha,0.816496580927726,1e-5);

			if (fabs(constants.constants.tau - 0.1) < 1e-5)
			{
				
				value_manuel[ID_theta] = -1.25115;
				value_manuel[ID_stress] = 0.00072895;
			...
			if (fabs(constants.constants.tau - 0.3) < 1e-5)
			{
				...
				value_manuel[ID_theta] = -1.33251;
				value_manuel[ID_stress] = -0.00761492;
				value_manuel[ID_heat] = -0.0455368;
		...
		value_manuel = matrix_opt.Sparse_matrix_dot_Vector(G84.base_tensorinfo.S_half,value_manuel);

		G84_PoissonHeat.vector_value(p,value);
