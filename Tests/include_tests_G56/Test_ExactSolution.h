namespace Test_ExactSolution
{
	...
	TEST(ExactSolutionPoissonHeatG56,HandlesExactSolutionG56)
	{
		...
		if (constants.constants.mesh_type == periodic_square)
		{
			...
			ASSERT_NEAR(G56.constants.alpha,0.816496580927726,1e-5);

			if (fabs(constants.constants.tau - 0.1) < 1e-5)
			{
				
				value_manuel[ID_theta] = -1.20988;
				value_manuel[ID_stress] = 0.00956687;
			...
			if (fabs(constants.constants.tau - 0.3) < 1e-5)
			{
				...
				value_manuel[ID_theta] = -1.24466;
				value_manuel[ID_stress] = 0.000780633;
				value_manuel[ID_heat] = -0.0455368;
		...
		value_manuel = matrix_opt.Sparse_matrix_dot_Vector(G56.base_tensorinfo.S_half,value_manuel);

		G56_PoissonHeat.vector_value(p,value);
