namespace Test_ExactSolution
{
	...
	TEST(ExactSolutionPoissonHeatG35,HandlesExactSolutionG35)
	{
		...
		if (constants.constants.mesh_type == periodic_square)
		{
			...
			ASSERT_NEAR(G35.constants.alpha,0.816496580927726,1e-5);

			if (fabs(constants.constants.tau - 0.1) < 1e-5)
			{
				
				value_manuel[ID_theta] = -1.27729;
				value_manuel[ID_stress] = 0.00093715;
			}

			if (fabs(constants.constants.tau - 0.3) < 1e-5)
			{
				
				value_manuel[ID_theta] = -1.40044;
				value_manuel[ID_stress] = 0.010108;
		...
		value_manuel = matrix_opt.Sparse_matrix_dot_Vector(G35.base_tensorinfo.S_half,value_manuel);

		G35_PoissonHeat.vector_value(p,value);
