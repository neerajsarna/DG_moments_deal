namespace Test_ForceType
{
	...
	TEST(ForceG26,HandlesForceG26)
	{
		...
		if (G26.constants.force_type == type3 && fabs(G26.constants.alpha - 1.0) < 1e-5)
		{
			std::vector<Point<dim>> p(1);

		// testing for forcetype1
			std::vector<Vector<double>> value(1,Vector<double>(G26.constants.nEqn));
		...
		// y coordinate
			...
			G26.source_term(p,value);
		// the exact value has been taken from mathematica file
			ASSERT_NEAR(value[0](dim + 1),-0.64,1e-5);			
		}
