namespace FEM_Solver
{
  ...
  // assembles hp FE with adaptive periodic
	template<int dim>
	class
	Assembly_Manager_hp_Periodic:public NumericalIntegration::Base_NumericalIntegration<dim>,
	...
	public:
		...
		Assembly_Manager_hp_Periodic(const std::string &output_file_name,
	...
	template<int dim>
	Assembly_Manager_hp_Periodic<dim>::Assembly_Manager_hp_Periodic(const std::string &output_file_name,
											                                         const constant_numerics &constants,
											                                         std::vector<Develop_System::System<dim>> &equation_info,
                        					                             const std::vector<int> &nEqn,
                        					                             const std::vector<int> &nBC, 
  ...
  PeriodicityHandler::Base_PeriodicityHandler<dim>(constants.xl,constants.xr),
	...
	template<int dim>
	void 
	Assembly_Manager_hp_Periodic<dim>::assemble_system_manuel()
	{
  ...
  for (; cell != endc ; cell++) 
      {
        ...
        integrate_cell_manuel(cell_matrix[fe_index],
            ...
            for(unsigned int face  = 0; face< GeometryInfo<dim>::faces_per_cell; face++ )
            {
              ...
              // initialise the FEValues on the face of this cell
              ...
              { 
                ...
                // if the face is periodic then we perform and integration over the internal edges    
                if(b_id == 20 || b_id == 21)
                { 
                  ...
                  // Get the iterator to the periodic neighbor.
                  neighbor = this->hp_get_periodic_neighbor(xcord_face_center,ycord_cell_center);
                 ...
                 std::vector<types::global_dof_index> local_dof_indices_neighbor(dofs_neighbor);
                  ...
                  // Get the face of the neighbor which is periodic.
                  const unsigned int neighbor_face = this->get_periodic_neighbor_face(xcord_face_center
                                                                                      ,ycord_cell_center);

                  
                  hp_fe_v_face_neighbor.reinit(neighbor,neighbor_face);
	...
	template<int dim>
	void 
	Assembly_Manager_hp_Periodic<dim>
