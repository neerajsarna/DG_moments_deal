...
template<int dim> 
void 
Base_Solver<dim>::assemble_system_char_eigen()
  {
      std::cout << "Assembling using eigen matrix " << std::endl;
      // counters 
      ...
      Assert(constants.matrix_type == Eigen_Mat,ExcMessage("Only for Eigen matrices"));
  ...
  for (; cell != endc ; cell++) 
        ...
        integrate_cell_manuel(cell_matrix,cell_rhs,
              ...
              { 
                ...
                for (unsigned int i = 0 ; i < dofs_per_cell ; i++)
                  for (unsigned int j = 0 ; j < dofs_per_cell ; j++)
                  {
                   ...
                   global_matrix_eigen.coeffRef(local_dof_indices[i],local_dof_indices[j])+=boundary_matrix(i,j);
                  }
              }
                ...
                else
               {
                 ...
                 if (cell->neighbor_is_coarser(face))
                 {
                  ...
                  for (unsigned int j = 0 ; j < dofs_per_cell ; j++)
                  {
                   ...
                   global_matrix_eigen.coeffRef(local_dof_indices[i],local_dof_indices[j]) += u1_v1(i,j);
                   global_matrix_eigen.coeffRef(local_dof_indices_neighbor[i],local_dof_indices_neighbor[j]) +=u2_v2(i,j);

                   global_matrix_eigen.coeffRef(local_dof_indices[i],local_dof_indices_neighbor[j]) += u2_v1(i,j);
                   global_matrix_eigen.coeffRef(local_dof_indices_neighbor[i],local_dof_indices[j]) += u1_v2(i,j);
                  }   
            ...
            for (unsigned int m = 0 ; m < cell_matrix.outerSize(); m++)
                for (Sparse_matrix::InnerIterator n(cell_matrix,m); n ; ++n)
                {
                  ...
                  global_matrix_eigen.coeffRef(local_dof_indices[n.row()],local_dof_indices[n.col()]) +=n.value();
                }
    ...
    global_matrix_eigen.makeCompressed();
...
template<int dim> 
void 
Base_Solver<dim>::assemble_system_odd_eigen()
  {
    std::cout << "Assembling using eigen matrix " << std::endl;
...
template<int dim> 
void 
Base_Solver<dim>::assemble_system_periodic_char_eigen()
  {
    std::cout << "Assembling using eigen matrix " << std::endl;
...
template<int dim> 
void 
Base_Solver<dim>::assemble_system_periodic_odd_eigen()
  {
    std::cout << "Assembling using eigen matrix " << std::endl;
      Assert(constants.mesh_type == periodic_square,ExcMessage("Present routine only for periodic squares"));
