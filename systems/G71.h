namespace G71
{
	...
	template<int dim>
	class
	Base_G71:public EquationGenerator::Base_EquationGenerator<dim>
	{
		public:
			Base_G71(const constant_data &constants,
						std::string &folder_name);
			

			// prepare varIdx for systemA, since we do not have a one to one correspondence with 
			const unsigned int Ntensors = 15;
	};
	...
	::G71(const constant_data &constants,
			  std::string &folder_name)
	:
	Base_G71<dim>(constants,folder_name),
