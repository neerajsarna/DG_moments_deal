namespace G84
{
	...
	template<int dim>
	class
	Base_G84:public EquationGenerator::Base_EquationGenerator<dim>
	{
		public:
			Base_G84(const constant_data &constants,
						std::string &folder_name);
			

			// prepare varIdx for systemA, since we do not have a one to one correspondence with 
			const unsigned int Ntensors = 16;
	};
	...
	::G84(const constant_data &constants,
			  std::string &folder_name)
	:
	Base_G84<dim>(constants,folder_name),
