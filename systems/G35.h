namespace G35
{
	...
	template<int dim>
	class
	Base_G35:public EquationGenerator::Base_EquationGenerator<dim>
	{
		public:
			Base_G35(const constant_data &constants,
						std::string &folder_name);
			

			// prepare varIdx for systemA, since we do not have a one to one correspondence with 
			const unsigned int Ntensors = 9;
	};
	...
	::G35(const constant_data &constants,
			  std::string &folder_name)
	:
	Base_G35<dim>(constants,folder_name),
