namespace G105
{
	...
	template<int dim>
	class
	Base_G105:public EquationGenerator::Base_EquationGenerator<dim>
	{
		public:
			Base_G105(const constant_data &constants,
						std::string &folder_name);
			

			// prepare varIdx for systemA, since we do not have a one to one correspondence with 
			const unsigned int Ntensors = 19;
	};
	...
	template<int dim>
	class
	G105:public Base_G105<dim>
	{
		public:
			// we need the input parameters to initialize the base class
			G105(const constant_data &constants,
					std::string &folder_name);
	...
	::G105(const constant_data &constants,
			  std::string &folder_name)
	:
	Base_G105<dim>(constants,folder_name),
