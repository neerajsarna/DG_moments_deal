...
namespace ExactSolution
{
	...
	template<>
	G45_PoissonHeat<2>::G45_PoissonHeat(const constant_data &constants,const Sparse_matrix &S_half)
	:
	...
	template<>
	G45_PoissonHeat<1>::G45_PoissonHeat(const constant_data &constants,const Sparse_matrix &S_half)
	:
	Base_ExactSolution<1>(constants,S_half)
	{
		Assert(constants.nEqn == 11,ExcMessage("Only a solution for G45"));
	}


	template<>
	void
	G45_PoissonHeat<2>::vector_value(const Point<2> &p,Vector<double> &value) const
	{
	...
	template<>
	void
	G45_PoissonHeat<1>::vector_value(const Point<1> &p,Vector<double> &value) const
	{
		// first we check the size of the value vector
		Assert((int)value.size() == this->constants.nEqn,ExcNotInitialized());
